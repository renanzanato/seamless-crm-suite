...
export function TopBar() {
  return (
    <header className="h-16 border-b border-border bg-card flex items-center justify-between px-6 sticky top-0 z-10">
      ...
      <button
        onClick={() => window.dispatchEvent(new KeyboardEvent('keydown', { key: 'f', ctrlKey: true, bubbles: true }))}
        className="relative max-w-md w-full"
      >
        <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" />
        <div className="w-full pl-10 pr-20 py-2 rounded-lg bg-secondary border border-border text-sm text-muted-foreground text-left cursor-pointer hover:border-primary/40 transition-colors">
          Buscar contatos, empresas, negócios...
        </div>
